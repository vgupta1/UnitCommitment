...
function solveRobust(rm::Model; report=false, active_cuts=false, args...)
    ...
    if isIP
        ...
        tic()
        ##VG
        get(prefs, :lprelax, false) && relax(master)
        haskey(prefs, :dumpfile) && writeLP(master, prefs[:dumpfile])
        
        lmo = get(prefs, :load_model_only, false)
        master_status = solve(master, load_model_only=lmo)


        #VG Add the gap onto the robust model
        if !lmo && !get(prefs, :lprelax, false)
            gap = (t=getInternalModel(master); (getobjval(t) - getobjbound(t))/getobjval(t))
            rm.ext[:Gap] = gap
        else
            rm.ext[:Gap] = 0.0
        end
